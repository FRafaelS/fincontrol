...
function App() {
  ...
  const buscarGastos = () => {
    fetchAuth(`${API_URL}/api/gastos`)
      ...
      .then((d) => { setGastos((d || []).map(g => ({
  ...g,
  valor_total: Number(g.valor_total || 0),
  valor_individual: Number(g.valor_individual || 0),
}))); setCarregando(false); })
  ...
  const handleChange = (e) => {
    ...
    setForm((prev) => {
      ...
      if (['tipo', 'responsavel', 'valor_total'].includes(name)) {
        ...
        if (t === 'I') novo.valor_individual = formatarMoeda(vt.toFixed(2));
        else if (t === 'C') {
          const resp = lkResponsavel.find((l) => l.MEANING === r);
          const div = resp?.TAG ? parseFloat(resp.TAG) : 1;
          novo.valor_individual = formatarMoeda((vt / div).toFixed(2));
        }
      }
      return novo;
    });
