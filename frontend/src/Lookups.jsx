  ...
  return (
    <div style={{ fontFamily: 'sans-serif', padding: '32px', maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: '4px', marginBottom: '24px', borderBottom: '2px solid #dee2e6' }}>
        <button
          onClick={() => { setAbaSelecionada('geral'); setMostrarForm(false); }}
          style={abaSelecionada === 'geral' ? abaAtiva : abaInativa}
        >
          Geral (Tipos)
        </button>
        <button
          onClick={() => { setAbaSelecionada('lookups'); setMostrarForm(false); }}
          style={abaSelecionada === 'lookups' ? abaAtiva : abaInativa}
        >
          Lookups (Valores)
        </button>
      </div>
        ...
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            <p style={{ margin: 0, color: '#666', fontSize: '14px' }}>
              Cadastre aqui os <strong>tipos</strong> de lookup. Eles estarão disponíveis na aba Lookups.
            </p>
        ...
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <select
                style={{ padding: '8px', borderRadius: '6px', border: '1px solid #ced4da', fontSize: '14px', minWidth: '220px' }}
                value={tipoSelecionado}
                onChange={(e) => { setTipoSelecionado(e.target.value); setMostrarForm(false); }}
              >
                <option value="">Selecione um tipo...</option>
                {tipos.map((t) => (
                  <option key={t.ID} value={t.LOOKUP_CODE}>{t.LOOKUP_CODE} — {t.MEANING}</option>
                ))}
              </select>
            </div>
            <button
              onClick={handleNovoFilho}
          ...
          {!tipoSelecionado && (
            <p style={{ color: '#999', fontSize: '14px' }}>Selecione um tipo para ver e cadastrar seus valores.</p>
          )}
...
function Formulario({ form, editandoId, salvando, erro, onChange, onSubmit, onCancelar, abaAtual, tipos }) {
  return (
    <form onSubmit={onSubmit} style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
        <div>
          <label style={label}>LOOKUP_TYPE *</label>
          <input style={input} name="LOOKUP_TYPE" value={form.LOOKUP_TYPE} onChange={onChange}
            readOnly={abaAtual === 'geral' || abaAtual === 'lookups'}
            required
          />
        </div>
        <div>
          <label style={label}>LOOKUP_CODE *</label>
          <input style={input} name="LOOKUP_CODE" value={form.LOOKUP_CODE} onChange={onChange} required />
        </div>
        <div>
          <label style={label}>MEANING *</label>
          <input style={input} name="MEANING" value={form.MEANING} onChange={onChange} required />
        </div>
        <div style={{ gridColumn: 'span 3' }}>
          <label style={label}>DESCRIPTION</label>
          <input style={input} name="DESCRIPTION" value={form.DESCRIPTION} onChange={onChange} />
        </div>
        <div>
          <label style={label}>TAG</label>
          <input style={input} name="TAG" value={form.TAG} onChange={onChange} />
        </div>
        <div>
          <label style={label}>ENABLED_FLAG</label>
          <select style={input} name="ENABLED_FLAG" value={form.ENABLED_FLAG} onChange={onChange}>
            <option value="S">S — Ativo</option>
            <option value="N">N — Inativo</option>
          </select>
        </div>
        <div>
          <label style={label}>ATTRIBUTE1</label>
          <input style={input} name="ATTRIBUTE1" value={form.ATTRIBUTE1} onChange={onChange} />
        </div>
        <div>
          <label style={label}>ATTRIBUTE2</label>
          <input style={input} name="ATTRIBUTE2" value={form.ATTRIBUTE2} onChange={onChange} />
        </div>
        <div>
          <label style={label}>ATTRIBUTE3</label>
          <input style={input} name="ATTRIBUTE3" value={form.ATTRIBUTE3} onChange={onChange} />
        </div>
      </div>

      <div style={{ display: 'flex', gap: '8px', marginTop: '20px' }}>
        <button type="submit" disabled={salvando} style={{
          background: editandoId ? '#fd7e14' : '#198754',
...
function Tabela({ dados, onEditar, onDeletar }) {
  if (dados.length === 0) {
    return <p style={{ color: '#999', fontSize: '14px' }}>Nenhum registro encontrado.</p>;
  }
    ...
    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
      <thead>
        <tr style={{ background: '#f5f5f5', textAlign: 'left' }}>
          <th style={th}>ID</th>
          <th style={th}>LOOKUP_TYPE</th>
          <th style={th}>LOOKUP_CODE</th>
      ...
      <tbody>
        {dados.map((l) => (
          <tr key={l.ID} style={{ borderBottom: '1px solid #eee' }}>
            <td style={td}>{l.ID}</td>
            <td style={td}>{l.LOOKUP_TYPE}</td>
            <td style={td}>{l.LOOKUP_CODE}</td>
            <td style={td}>{l.MEANING}</td>
            <td style={td}>{l.DESCRIPTION || '—'}</td>
            <td style={td}>
              <span style={{
                background: l.ENABLED_FLAG === 'S' ? '#d4edda' : '#f8d7da',
                color: l.ENABLED_FLAG === 'S' ? '#155724' : '#842029',
