...
// 🔥 NOVO: função para tratar moeda brasileira
const parseBR = (v) => {
  if (!v) return 0;
  return parseFloat(
    String(v)
  ...
  const gerarPreview = () => {
    const registros = linhas.map((linha) => {
      ...
      Object.entries(mapeamento).forEach(([coluna, campo]) => {
        if (campo) {
          registro[campo] = campo === 'data_venc' ? converterDataExcel(linha[coluna]) : linha[coluna];
        }
      });
      return registro;
    });
...
if (data.erro) { erros++; errosList.push(data.erro); } else sucesso++;
      } catch { erros++; errosList.push(`Erro ao importar: ${registro.descricao}`); }
    }
    ...
    <div style={{ fontFamily: 'sans-serif', padding: '32px', maxWidth: '1100px', margin: '0 auto' }}>
        ...
        {['Upload', 'Mapeamento', 'Revisão', 'Resultado'].map((e, i) => {
          ...
          return (
            <div key={e} style={{ flex: 1, textAlign: 'center' }}>
              <div style={{ padding: '8px', background: concluido ? '#198754' : ativo ? '#0d6efd' : '#e9ecef', color: ativo || concluido ? '#fff' : '#6c757d', fontSize: '13px', fontWeight: '600', borderRadius: i === 0 ? '6px 0 0 6px' : i === 3 ? '0 6px 6px 0' : '0' }}>
                {concluido ? '✓ ' : ''}{e}
              </div>
            </div>
          );
      ...
      {etapa === 'upload' && (
        <div style={{ textAlign: 'center', padding: '48px', border: '2px dashed #dee2e6', borderRadius: '12px' }}>
          ...
          <p style={{ color: '#666', marginBottom: '24px' }}>Formatos suportados: <strong>.xlsx</strong> e <strong>.xls</strong></p>
          <label style={{ background: '#0d6efd', color: '#fff', border: 'none', borderRadius: '6px', padding: '10px 24px', cursor: 'pointer', fontSize: '14px' }}>
            Escolher arquivo
            <input type="file" accept=".xlsx,.xls" onChange={handleArquivo} style={{ display: 'none' }} />
          </label>
          {erroArquivo && <p style={{ color: '#dc3545', marginTop: '16px', fontSize: '14px' }}>{erroArquivo}</p>}
          <div style={{ marginTop: '32px', background: '#f8f9fa', borderRadius: '8px', padding: '16px', textAlign: 'left' }}>
            <p style={{ margin: '0 0 8px', fontWeight: '600', fontSize: '14px' }}>Colunas sugeridas:</p>
            <code style={{ fontSize: '13px', color: '#666' }}>RESP | TIPO | DESCRICAO | PARCELA | CATEGORIA | FORMA_PGTO | VALOR_TOTAL | DATA_VENC | MES | ANO | STATUS | OBS</code>
          </div>
        </div>
      )}
      ...
      {etapa === 'mapeamento' && (
        <>
          <div style={{ background: '#f8f9fa', border: '1px solid #dee2e6', borderRadius: '8px', padding: '20px', marginBottom: '24px' }}>
            ...
            <p style={{ margin: 0, color: '#666', fontSize: '14px' }}>{linhas.length} linha(s) · {colunas.length} coluna(s)</p>
          </div>
      ...
      {etapa === 'revisao' && (
        <>
          <div style={{ background: '#fff3cd', border: '1px solid #ffc107', borderRadius: '8px', padding: '16px', marginBottom: '24px' }}>
            <p style={{ margin: 0, fontWeight: '600', color: '#856404' }}>⚠️ Revise os dados — {preview.length} registro(s) serão criados</p>
          </div>
          <div style={{ overflowX: 'auto', marginBottom: '24px' }}>
            ...
            {preview.length > 20 && <p style={{ color: '#666', fontSize: '13px', padding: '8px 12px' }}>Mostrando 20 de {preview.length} registros...</p>}
          </div>
          <div style={{ display: 'flex', gap: '8px' }}>
      ...
      {etapa === 'resultado' && resultado && (
        <div style={{ textAlign: 'center', padding: '32px' }}>
          ...
          <h2 style={{ fontSize: '20px', marginBottom: '16px' }}>{resultado.erros === 0 ? 'Importação concluída!' : 'Concluída com avisos'}</h2>
          ...
          {resultado.errosList.length > 0 && (
            <div style={{ background: '#f8d7da', borderRadius: '8px', padding: '16px', marginBottom: '24px', textAlign: 'left' }}>
              {resultado.errosList.map((e, i) => <p key={i} style={{ margin: '4px 0', fontSize: '13px', color: '#842029' }}>• {e}</p>)}
            </div>
          )}
