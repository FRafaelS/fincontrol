...
const campoVazio = {
  ...
  valor_total: '', total_parcelas: '', dia_vencimento: '',
  mes_inicial: '', ano_inicial: new Date().getFullYear(), status: '', obs: '',
};

function Parcelas({ onVoltar, token }) {
  ...
  useEffect(() => {
    const buscar = (tipo, setter) =>
      fetch(`${API_URL}/api/lookups/valores/${tipo}`, { headers })
        .then((r) => r.json()).then((d) => Array.isArray(d) && setter(d)).catch(() => setter([]));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/parcelas/gerar`, {
        method: 'POST', headers,
        body: JSON.stringify({
          ...form, valor_total: parseFloat(form.valor_total),
    ...
    <div style={{ fontFamily: 'sans-serif', padding: '32px', maxWidth: '900px', margin: '0 auto' }}>
      ...
      {resultado && (
        <div style={{ background: '#d4edda', border: '1px solid #c3e6cb', borderRadius: '8px', padding: '16px', marginBottom: '24px' }}>
          <p style={{ margin: 0, fontWeight: '600', color: '#155724' }}>🎉 {resultado.total} parcela(s) gerada(s) com sucesso!</p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={secao}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
              <div style={{ gridColumn: 'span 3' }}><label style={label}>Descrição *</label><input style={input} name="descricao" value={form.descricao} onChange={handleChange} required /></div>
          ...
          <div style={secao}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '16px' }}>
              ...
              <div>
                ...
                <input style={{ ...input, background: '#e9ecef', cursor: 'not-allowed' }} value={form.valor_individual || ''} readOnly tabIndex={-1} />
                <span style={{ fontSize: '11px', color: '#888', marginTop: '4px', display: 'block' }}>
                  {form.tipo === 'C' && form.responsavel ? `Total ÷ ${lkResponsavel.find(l => l.MEANING === form.responsavel)?.TAG || 1}` : form.tipo === 'I' ? 'Igual ao valor total' : '—'}
                </span>
              </div>
            ...
            <button type="button" onClick={gerarPreview} style={{ ...btnPrimario, marginTop: '16px', background: '#6f42c1' }}>👁 Visualizar Parcelas</button>
          </div>
            ...
            <div style={secao}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
                <thead><tr style={{ background: '#f5f5f5' }}>
                  <th style={th}>Parcela</th><th style={th}>Mês</th><th style={th}>Ano</th><th style={th}>Vencimento</th><th style={th}>Valor Individual</th>
                </tr></thead>
                <tbody>
                  {preview.map((p, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #eee', background: i === 0 ? '#e8f4fd' : 'transparent' }}>
                      <td style={td}>{p.parcela}</td><td style={td}>{p.mesAtual}</td>
              ...
              <button type="submit" disabled={salvando} style={{ ...btnPrimario, marginTop: '20px', background: '#198754' }}>
